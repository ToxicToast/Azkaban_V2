...
export function HeaderRightSidePartial(props: PropsWithChildren<Props>) {
  const onSearchModalToggle = useCallback(
      ...
      <div>
        <button
          className={`w-8 h-8 flex items-center justify-center bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600/80 rounded-full ml-3 ${
            props.searchModalOpen && 'bg-slate-200'
          }`}
          onClick={(e) => onSearchModalToggle(e)}
        >
